...
function slopeRhs(m: Exact, c: Exact): string {
  let rhs = '';
  if (!m.isZero()) rhs = m.equals(E(1)) ? 'x' : m.equals(E(-1)) ? '-x' : `${m.toLatex(FR)}x`;
  if (!c.isZero()) {
...
function tangentGradientQ(rng: RNG): Generated | null {
  ...
  return {
    ...
    options: options(rng, answer, [
      ...
      { value: frac(-1, m), trap: 'gave the gradient of the normal' },
    ], [
      { value: E(noLower(coefs, a)), trap: 'applied the power rule without lowering the power' },
      { value: E(-m), trap: 'sign slip' },
      { value: E(evalPoly(dCoefs(coefs), -a)), trap: 'substituted x = −a' },
    ]),
...
function normalGradientQ(rng: RNG): Generated | null {
  ...
  const { coefs, a, y0, m } = c;
  if (m === 0 || Math.abs(m) === 1 || Math.abs(m) > 40) return null;
  ...
  return {
    ...
    options: options(rng, answer, [
      ...
      { value: frac(1, m), trap: 'took the reciprocal but did not change the sign' },
    ], [
      { value: E(m), trap: 'gave the gradient of the tangent' },
      { value: y0 !== 0 ? frac(-1, y0) : null, trap: 'used y instead of dy/dx as the tangent gradient' },
      { value: noLower(coefs, a) !== 0 ? frac(-1, noLower(coefs, a)) : null, trap: 'power rule without lowering the power' },
    ]),
    solution: `$\\frac{dy}{dx} = ${poly(dCoefs(coefs))} = ${m}$ at $x = ${a}$, so the normal has gradient $${m > 0 ? answer.toLatex(FR) : `-\\frac{1}{${m}} = ${answer.toLatex(FR)}`}$.`,
...
function tangentEquationQ(rng: RNG): Generated | null {
  ...
  const cands: { m: Exact; c: Exact; trap: string }[] = [
    { m: frac(-1, m), c: E(y0).add(frac(a, m)), trap: 'this is the normal, not the tangent' },
...
function tangentKQ(rng: RNG): Generated | null {
  ...
  const distractors = [
    { values: [E(q - s), E(q + s)], trap: 'forgot the factor 2: q − k = ±2s, not ±s' },
    ...
    { values: [E(q)], trap: 'took the discriminant as giving a single value of k' },
  ...
  return {
    ...
    options: buildSetOptions(rng, values, distractors, { variable: 'k' }),
