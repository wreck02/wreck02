...
function momentOfForce(rng: RNG): Generated | null {
  ...
  return pack(rng, {
    ...
    extra: [
      ...
      { value: F2 * d1, trap: 'used the other force with this force’s distance' },
      { value: F1 > F2 ? (F1 - F2) * d1 : null, trap: 'used the resultant of the two forces instead of the one asked for' },
      { value: 2 * M, trap: 'added the two moments instead of giving one of them' },
      { value: M / 2, trap: 'halved the moment' },
      { value: cm ? M / 10 : null, trap: 'divided by 1000, as if the distance were in millimetres' },
...
function beamOnPivot(rng: RNG, ask: 'f' | 'x'): Generated | null {
  ...
  const extra: Cand[] = ask === 'f'
    ...
    : [
        ...
        { value: round(p - (wb * (L - p)) / wl), trap: 'took the beam’s weight to act at $B$ instead of at its centre' },
...
function twoLoads(rng: RNG): Generated | null {
  ...
  return pack(rng, {
    ...
    extra: [
      ...
      { value: (Wb * (L / 2) + W1 * a1) / L, trap: `left the load $${n(d2)}\\ \\text{m}$ from $A$ out of the moment equation` },
      { value: (Wb * (L / 2) + W2 * a2) / L, trap: `left the load $${n(d1)}\\ \\text{m}$ from $A$ out of the moment equation` },
