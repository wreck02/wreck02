...
function options(rng: RNG, answer: Exact, must: Candidate[], extra: Candidate[], unit: string) {
  return buildOptions(rng, answer, ranked(rng, answer, cleanOnly(must), cleanOnly(extra)), { format: 'fraction', unit });
}
...
function displacementQ(rng: RNG): Generated | null {
  ...
  return {
    ...
    options: options(rng, s, must, extra, M),
...
function velocityQ(rng: RNG): Generated | null {
  ...
  return {
    ...
    options: options(rng, vT, must, extra, MS),
...
function restQ(rng: RNG): Generated | null {
  ...
  let v: Poly, k: number, p: number;
  if (kind === 't2-c') {
    k = rng.pick([1, 2, 2, 3]); p = rng.pick([1, 3, 3, 6, 2]);
    v = [p, 0, -p * k * k];
  } else {
    k = rng.pick([2, 3, 4]); p = rng.pick([1, 3, 3, 6]);
    v = [p, -p * k, 0];
  }
  const s = defInt(v, 0, k);
  if (!isCleanExact(s).ok || Math.abs(s.toNumber()) > 120 || s.toRat().d > 3n) return null;
  const q = -v[v.length - 1] || -v[1];
  const must = positive([
    { value: s.neg(), trap: 'gave the distance travelled (or dropped the sign): the particle has moved in the negative direction' },
    { value: E(k), trap: 'gave the time at which the particle is at rest, not the displacement' },
  ]);
  const extra = [
    { value: noDivInt(v, 0, k), trap: 'did not divide by the new powers when integrating' },
    { value: defInt([v[0], 0, 0].slice(0, v.length), 0, k), trap: 'integrated only the t² term' },
    { value: kind === 't2-c' ? E(-q * k) : defInt([0, v[1], 0], 0, k), trap: 'dropped the t² term' },
    { value: kind === 't2-c' && q / p !== k ? defInt(v, 0, q / p) : null, trap: `solved ${p}t² = ${q} as t = ${q / p} instead of t = ${k}` },
    { value: s.mulRat(2), trap: 'doubled the displacement' },
    { value: horner(v, 0) !== 0 ? E(horner(v, 0)) : null, trap: 'gave the initial velocity' },
  ];
  return {
    stem: `A particle moves along a straight line so that its velocity at time $t$ seconds is ${V(v)}. The particle is at the origin $O$ when $t = 0$. Find its displacement from $O$ at the instant when it first comes to instantaneous rest.`,
    answer: { kind: 'exact', value: s, unit: M },
    options: options(rng, s, must, extra, M),
    solution: `$v = 0$ when $${nice(v)} = 0$, i.e. $t = ${k}$ (taking $t > 0$). Then $s = \\int_{0}^{${k}} (${nice(v)})\\,dt = \\left[${antiTex(v)}\\right]_{0}^{${k}} = ${L(s)}$ m.`,
    trap: 'First solve v = 0 for t, then integrate v from 0 to that time; the displacement is negative because the particle moved backwards.',
...
function distanceQ(rng: RNG): Generated | null {
  const kind = rng.pick(['t2-m2', 't2-m2', 'linear', 'linear-dec']);
  let v: Poly, root: number, T: number;
  if (kind === 't2-m2') {
    const m = rng.pick([1, 2, 2, 3]);
    v = [1, 0, -m * m]; root = m; T = m + rng.pick([1, 2]);
  } else {
    const p = rng.pick([1, 2, 3, 2]);
  ...
  const extra = positive([
    ...
    { value: A1.sub(A2).abs(), trap: 'sign of the second stage wrong' },
  ...
  return {
    ...
    options: options(rng, dist, must, extra, M),
...
function returnQ(rng: RNG): Generated | null {
  ...
  const extra = positive([
    ...
    { value: E(tR + 1), trap: 'arithmetic slip' },
    { value: E(tR - 1), trap: 'arithmetic slip' },
  ...
  return {
    ...
    options: options(rng, answer, must, extra, S),
...
function twoStageQ(rng: RNG): Generated | null {
  ...
  return {
    ...
    options: options(rng, total, must, extra, M),
