 ...
 * Level 1: one prefix — km → m, mA → A, MW → W, µs → s, GHz → Hz
 * Level 2: squared and cubed units — cm² → m² (÷10⁴), cm³ → m³ (÷10⁶), mm → km
...
function pack(rng: RNG, p: Pack): Generated | null {
  ...
  const ds = ranked(rng, value, cleanOnly(p.must, format), cleanOnly(p.extra, format), p.spread ?? 1e6);
...
const LEVEL1: Recipe[] = [
  {
    ...
    values: [0.4, 1.2, 2.5, 3, 4.5, 7.5, 12, 25, 40], quantity: 'distance',
    ctx: (v) => `A stretch of road is $${v}\\ \\text{km}$ long.`,
    wrong: [{ m: 1e-3, trap: 'divided by $10^{3}$ instead of multiplying' }, { m: 1e6, trap: 'used the prefix mega instead of kilo' }, { m: 1e2, trap: 'confused kilo with a factor of 100' }],
  },
  {
    from: 'mA', to: 'A',
    ...
    values: [5, 20, 40, 50, 250, 400, 750, 1500], quantity: 'current',
  ...
  },
  {
    ...
    values: [2, 2.5, 5, 8, 12, 25, 400, 600, 800], quantity: 'power',
  ...
  },
  {
    ...
    values: [4, 20, 25, 40, 50, 250, 500], quantity: 'time',
    ctx: (v) => `A pulse of light lasts $${v}\\ \\mu\\text{s}$.`,
    wrong: [{ m: 1e-3, trap: 'confused micro ($10^{-6}$) with milli ($10^{-3}$)' }, { m: 1e6, trap: 'multiplied by $10^{6}$ instead of dividing' }, { m: 1e-9, trap: 'used nano instead of micro' }],
  },
  {
    from: 'GHz', to: 'Hz',
    trapLine: 'Giga is $10^{9}$: three more powers of ten than mega.', fromTex: '\\text{GHz}', toTex: '\\text{Hz}', words: 'in hertz', mult: 1e9,
    values: [1.5, 2, 2.5, 3, 4, 5], quantity: 'frequency',
    ctx: (v) => `A radio transmitter works at a frequency of $${v}\\ \\text{GHz}$.`,
    wrong: [{ m: 1e6, trap: 'used mega instead of giga' }, { m: 1e-9, trap: 'divided by $10^{9}$ instead of multiplying' }, { m: 1e12, trap: 'used $10^{12}$ for giga' }],
  },
  {
    from: 'kJ', to: 'J',
    ...
    values: [1.5, 2.5, 6, 12, 45, 80, 250], quantity: 'energy',
    ctx: (v) => `A kettle transfers $${v}\\ \\text{kJ}$ of energy.`,
    wrong: [{ m: 1e-3, trap: 'divided by $10^{3}$ instead of multiplying' }, { m: 1e6, trap: 'used mega instead of kilo' }],
  },
];
...
const LEVEL2: Recipe[] = [
  {
    ...
    values: [50, 80, 120, 250, 400, 500, 2500], quantity: 'area',
    ctx: (v) => `A metal plate has an area of $${v}\\ \\text{cm}^{2}$.`,
    wrong: [{ m: 1e-2, trap: 'divided by $10^{2}$: an area needs the length factor squared' }, { m: 1e-6, trap: 'used the volume factor $10^{6}$' }, { m: 1e4, trap: 'multiplied by $10^{4}$ instead of dividing' }],
  },
  {
    from: 'cm^3', to: 'm^3',
    ...
    values: [200, 250, 400, 500, 1500, 2500, 4000], quantity: 'volume',
    ctx: (v) => `A container holds $${v}\\ \\text{cm}^{3}$ of liquid.`,
    wrong: [{ m: 1e-3, trap: 'divided by $10^{3}$: a volume needs the length factor cubed' }, { m: 1e-4, trap: 'used the area factor $10^{4}$' }, { m: 1e6, trap: 'multiplied by $10^{6}$ instead of dividing' }],
  },
  {
    from: 'mm^2', to: 'm^2',
    ...
    values: [200, 400, 500, 2500, 4000, 5000], quantity: 'area',
    ctx: (v) => `The cross-section of a wire has an area of $${v}\\ \\text{mm}^{2}$.`,
    wrong: [{ m: 1e-3, trap: 'divided by $10^{3}$: an area needs the length factor squared' }, { m: 1e-4, trap: 'used the factor for cm² instead of mm²' }, { m: 1e6, trap: 'multiplied instead of dividing' }],
  },
  {
    from: 'mm', to: 'km',
    ...
    values: [2500, 4000, 7500, 25000, 40000], quantity: 'length',
    ctx: (v) => `A length of wire measures $${v}\\ \\text{mm}$.`,
    wrong: [{ m: 1e-3, trap: 'only one of the two conversions done' }, { m: 1e-9, trap: 'a factor of $10^{3}$ too many' }, { m: 1e6, trap: 'multiplied instead of dividing' }],
  },
];
...
const LEVEL3: Recipe[] = [
  {
    ...
    values: [18, 36, 54, 72, 90, 108, 126, 144], quantity: 'speed',
    ctx: (v) => `A car travels at $${v}\\ \\text{km/h}$.`,
    wrong: [{ m: 18 / 5, trap: 'multiplied by 3.6 instead of dividing by it' }, { m: 1e3 / 60, trap: 'divided by 60 instead of 3600' }, { m: 1, trap: 'left the number unchanged' }],
  ...
  },
  {
    ...
    values: [5, 10, 12, 15, 20, 25, 30, 40], quantity: 'speed',
    ctx: (v) => `A cyclist rides at $${v}\\ \\text{m s}^{-1}$.`,
    wrong: [{ m: 5 / 18, trap: 'divided by 3.6 instead of multiplying by it' }, { m: 60 / 1e3, trap: 'used 60 seconds in an hour' }, { m: 1, trap: 'left the number unchanged' }],
  ...
  },
  {
    ...
    values: [0.8, 1.2, 2.5, 2.7, 7.8, 8.9, 11.3, 13.6], quantity: 'density',
    ctx: (v) => `A metal has a density of $${v}\\ \\text{g cm}^{-3}$.`,
    wrong: [{ m: 1e-3, trap: 'divided by $10^{3}$ instead of multiplying' }, { m: 1e6, trap: 'used $10^{6}$ (the volume factor alone)' }, { m: 1e-6, trap: 'both conversions the wrong way round' }],
  },
];
...
const LEVEL5_RECIPES: Recipe[] = [
  ...
  },
  {
    ...
    values: [2, 5, 20, 40, 250, 500], quantity: 'mass',
    ctx: (v) => `A speck of dust has a mass of $${v}\\ \\mu\\text{g}$.`,
    wrong: [{ m: 1e-6, trap: 'converted micrograms to grams and stopped' }, { m: 1e-12, trap: 'one conversion too many' }, { m: 1e9, trap: 'multiplied instead of dividing' }],
  },
  {
    from: 'kW h', to: 'J',
    ...
    values: [0.5, 1, 1.5, 2, 3, 5], quantity: 'energy',
    ctx: (v) => `An electricity meter records $${v}\\ \\text{kW h}$.`,
    wrong: [{ m: 3.6e3, trap: 'forgot to turn kilowatts into watts' }, { m: 1e3 * 60, trap: 'used 60 seconds in an hour' }, { m: 1e3, trap: 'treated one hour as one second' }],
  },
];
...
function recipeQ(rng: RNG, pool: Recipe[]): Generated | null {
  ...
  return pack(rng, {
    ...
    extra: [
      ...
      { value: round(a * 10), trap: 'a single power of ten out' },
      { value: round(a / 10), trap: 'a single power of ten out' },
...
function energyQ(rng: RNG): Generated | null {
  ...
  return pack(rng, {
    ...
    extra: [
      ...
      { value: round(a * 10), trap: 'a power of ten out' },
      { value: round(a / 10), trap: 'a power of ten out' },
    ],
...
export default defineTemplate({
  ...
  levels: {
    1: 'one prefix: km → m, mA → A, MW → W, µs → s, GHz → Hz',
    2: 'squared and cubed units: cm² → m² (÷10⁴), cm³ → m³ (÷10⁶), mm → km',
